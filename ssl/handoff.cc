...
static bool SSL_deserialize_entire_state_(SSL *ssl, Span<const uint8_t> handback) {
...
// XXX - JEDI s3->hs = ssl_handshake_new(ssl);
  SSL_HANDSHAKE *const hs = s3->hs.get();
...
extern "C" {
#endif
    int SSL_deserialize_entire_state(SSL *ssl,
                                     const uint8_t *handback,
                                     const size_t handback_len) {
        return SSL_deserialize_entire_state_(ssl, Span<const uint8_t>(handback, handback_len));
    }
#if defined(__cplusplus)
}
#endif
